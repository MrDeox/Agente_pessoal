"""
OpenRouter Provider for LLM Integration

This module contains the OpenRouter provider implementation, which is compatible with the OpenAI API.
"""

from typing import List
from .base import LLMProvider
from ..models import Message, LLMResponse
from ..exceptions import LLMException, RateLimitError, AuthenticationError, ModelError, InvalidRequestError, NetworkError, ServiceUnavailableError, TimeoutError, ContextLengthError, ContentPolicyError, QuotaExceededError
...
class OpenRouterProvider(LLMProvider):
    ...
    def _handle_openrouter_exception(self, e: Exception) -> Exception:
        """
        Convert OpenRouter API exceptions to our specific exception types.
        ...
        """
        # Handle OpenAI-compatible exceptions if available
        if hasattr(openai, 'AuthenticationError') and isinstance(e, openai.AuthenticationError):
            return AuthenticationError(f"OpenRouter API authentication failed: {str(e)}")
        elif hasattr(openai, 'RateLimitError') and isinstance(e, openai.RateLimitError):
            return RateLimitError(f"OpenRouter API rate limit exceeded: {str(e)}")
        elif hasattr(openai, 'NotFoundError') and isinstance(e, openai.NotFoundError):
            return ModelError(f"OpenRouter model not found: {str(e)}")
            ...
            if "context_length" in str(e).lower() or "too long" in str(e).lower():
                return ContextLengthError(f"OpenRouter API context length exceeded: {str(e)}")
            return InvalidRequestError(f"Invalid request to OpenRouter API: {str(e)}")
        elif hasattr(openai, 'PermissionDeniedError') and isinstance(e, openai.PermissionDeniedError):
            return ContentPolicyError(f"OpenRouter API content policy violation: {str(e)}")
        elif hasattr(openai, 'APIConnectionError') and isinstance(e, openai.APIConnectionError):
            return NetworkError(f"Network error connecting to OpenRouter API: {str(e)}")
        elif hasattr(openai, 'APIStatusError') and isinstance(e, openai.APIStatusError):
            status_code = getattr(e, 'status_code', 0)
            if status_code == 503:
                return ServiceUnavailableError(f"OpenRouter API service unavailable: {str(e)}")
            elif status_code == 408:
                return TimeoutError(f"OpenRouter API request timeout: {str(e)}")
            elif status_code == 429:
                return QuotaExceededError(f"OpenRouter API quota exceeded: {str(e)}")
            else:
                return LLMException(f"OpenRouter API error (status {status_code}): {str(e)}")
        else:
            ...
            if "authentication" in error_str or "api key" in error_str:
                ...
                return ModelError(f"OpenRouter model error: {str(e)}")
                ...
                return LLMException(f"OpenRouter API error: {str(e)}")
    ...
    def generate_response(self, messages: List[Message], **kwargs) -> LLMResponse:
        ...
        Returns:
            LLMResponse: Response from OpenRouter
            
        Raises:
            LLMException: If there's an error with the OpenRouter API
        """
        logger = get_logger()
        ...
        def _make_api_call():
            logger.info(f"Calling OpenRouter API with model: {self.config.model or 'qwen/qwen3-coder:free'}")
            response = self.client.chat.completions.create(
                model=self.config.model or "qwen/qwen3-coder:free",
        ...
        try:
            response = retry_function(
                _make_api_call,
                retry_config,
                retryable_exceptions=(LLMException, RateLimitError),
                logger=logger
            )
            ...
            logger.error("OpenRouter API model error")
            raise
        except Exception as e:
            log_exception(e, "OpenRouter API call")
            raise self._handle_openrouter_exception(e)
    ...
    def stream_response(self, messages: List[Message], **kwargs):
        ...
        def _make_streaming_api_call():
            logger.info(f"Streaming from OpenRouter API with model: {self.config.model or 'qwen/qwen3-coder:free'}")
            response = self.client.chat.completions.create(
                model=self.config.model or "qwen/qwen3-coder:free",
                messages=self._convert_messages(messages),
                temperature=self.config.temperature,
        ...
        try:
            ...
            logger.info("OpenRouter API streaming completed successfully")
        except RateLimitError:
            logger.error("OpenRouter API rate limit exceeded")
            raise
        except AuthenticationError:
            ...
            log_exception(e, "OpenRouter API streaming")
            raise self._handle_openrouter_exception(e)
    ...
    async def generate_response_async(self, messages: List[Message], **kwargs) -> LLMResponse:
        ...
        try:
            ...
            log_exception(e, "OpenRouter async API call")
            raise self._handle_openrouter_exception(e)
    ...
    async def stream_response_async(self, messages: List[Message], **kwargs):
        ...
        try:
            ...
            log_exception(e, "OpenRouter async API streaming")
            raise self._handle_openrouter_exception(e)